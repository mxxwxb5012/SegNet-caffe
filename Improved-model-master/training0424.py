...
def convolution_unit(input_layer, k, pad, stride, planes, dilation,lr_mult=1, decay_mult=1):
    ...
    if dilation==1:
        conv = L.Convolution(input_layer,
                            ...
                            {'lr_mult': 2*lr_mult,'decay_mult':0}] 
                            )
...
def convolution_block(net, input_layer, base_name, layers, k=3, pad=1,\
                      stride=1,planes=(64,64,64), dilation=1,lr_mult=1, decay_mult=1, reverse=False):
    ...
    for idx, i in enumerate(range_):
        if idx == 0:
            in_ = input_layer
        conv,  relu = convolution_unit(in_, k, pad, stride, planes[3-i], dilation,lr_mult=lr_mult, decay_mult=decay_mult)
...
def segnet_network(data_source, label_source, mode='train'):
    ...
    convolution_block(n, n.data, "conv1_{}", 2, planes=(64,64,64), lr_mult=1)
    n.pool1, n.pool1_mask = L.Pooling(n.conv1_2, pool=P.Pooling.MAX, kernel_size=2, stride=2, ntop=2)

    convolution_block(n, n.pool1, "conv2_{}", 2, planes=(128,128,128), lr_mult=1)
    n.pool2, n.pool2_mask = L.Pooling(n.conv2_2, pool=P.Pooling.MAX, kernel_size=2, stride=2, ntop=2)

    convolution_block(n, n.pool2, "conv3_{}", 3, planes=(256,256,256), lr_mult=1)
    n.pool3, n.pool3_mask = L.Pooling(n.conv3_3, pool=P.Pooling.MAX, kernel_size=2, stride=2, ntop=2)

    convolution_block(n, n.pool3, "conv4_{}", 3, planes=(512,512,512), lr_mult=1)
    n.pool4 = L.Pooling(n.conv4_3, pool=P.Pooling.MAX, kernel_size=3, stride=1, pad=1, ntop=1)

    convolution_block(n, n.pool4, "conv5_{}", 3, pad=2,planes=(512,512,512),dilation=2, lr_mult=1)

    n.pool5 = L.Pooling(n.conv5_3, pool=P.Pooling.MAX, kernel_size=3, stride=1, pad=1, ntop=1)
    
    n.conv5_3_aspp1_conv, n.conv5_3_aspp1_relu=convolution_unit(n.pool5,1,0,1,512,1,lr_mult=1)
   
    n.conv5_3_aspp2_conv, n.conv5_3_aspp2_relu=convolution_unit(n.pool5,3,1,1,512,1,lr_mult=1)
   
    n.conv5_3_aspp3_conv, n.conv5_3_aspp3_relu=convolution_unit(n.pool5,5,2,1,512,1,lr_mult=1)
    
    n.conv5_3_aspp4_conv, n.conv5_3_aspp4_relu=convolution_unit(n.pool5,7,3,1,512,1,lr_mult=1)

    n.conv5_3_aspp_concat=L.Concat(n.conv5_3_aspp1_conv,n.conv5_3_aspp2_conv,n.conv5_3_aspp3_conv,n.conv5_3_aspp4_conv)

    n.conv5_4_conv, n.conv5_4_relu=convolution_unit(n.conv5_3_aspp_concat,1,0,1,256,1,lr_mult=1)

    n.score_pool5 = L.Convolution(n.pool5, num_output=256, kernel_size=1, pad=0,param=dict(lr_mult=0.5, decay_mult=1))
  #  n.score_pool3c = L.Crop(n.score_pool2, n.upsample3, crop_param=dict(axis=2,offset=0))
    n.fuse_pool4 = L.Concat(n.conv5_4_conv, n.score_pool5)
    convolution_block(n, n.fuse_pool4, "conv4_{}_D", 3, planes=(512,512,256), lr_mult=1, reverse=True)

    n.upsample3 = L.Upsample(n.conv4_1_D, n.pool3_mask, scale=2)
    n.score_pool2 = L.Convolution(n.pool2, num_output=256, kernel_size=1, pad=0,param=dict(lr_mult=0.5, decay_mult=1))
    n.score_pool3c = L.Crop(n.score_pool2, n.upsample3,crop_param=dict(axis=2,offset=0))
    n.fuse_pool3 = L.Eltwise(n.upsample3, n.score_pool3c,operation=P.Eltwise.SUM)

    convolution_block(n, n.fuse_pool3, "conv3_{}_D", 3, planes=(256,256,128), lr_mult=1, reverse=True)

    n.upsample2 = L.Upsample(n.conv3_1_D, n.pool2_mask, scale=2)
    # n.score_pool1 = L.Convolution(n.pool1, num_output=128, kernel_size=1, pad=0,param=dict(lr_mult=0.5, decay_mult=1))
    # n.score_pool2c = L.Crop(n.score_pool1, n.upsample2,crop_param=dict(axis=2,offset=0))
    # n.fuse_pool2 = L.Eltwise(n.upsample2, n.score_pool2c,operation=P.Eltwise.SUM)
    ...
    n.conv1_2_D,  n.conv1_2_D_relu =\
                                convolution_unit(n.upsample1, 3, 1,1, 64, 1,lr_mult=1)
    n.conv1_1_D, _ = convolution_unit(n.conv1_2_D, 3, 1, 1,22,1, lr_mult=10)#最后一层为类别分类层，4表示类别数目

    # n.upsample1 = L.Deconvolution(n.conv2_1_D,
    #     convolution_param=dict(num_output=21, kernel_size=4, stride=2,
    #         bias_term=False),
    #     param=[dict(lr_mult=0)])

    #n.score = L.Crop(n.upsample1, n.data)
